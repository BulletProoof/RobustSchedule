...
class Model:
    ...
    def add_variables(self):
        ...
        for i in range(self.instance.N):
            for j in range(self.instance.N):
                for h in range(self.instance.H):
    ...
    def add_constraints(self):
        ...
        for j in range(1, self.instance.N):
            self.model.addConstr(quicksum(self.X[i, j] for i in range(self.instance.N)) == 1,"c1:visit_once_in_[%self.d]" % j)
            self.model.addConstr(quicksum(self.X[j, i] for i in range(self.instance.N)) == 1,"c1:visit_once_out_[%self.d]" % j)

        # 约束（2）：车辆流量平衡约束-车辆从配送中心出发必须返回配送中心
        for j in range(0, self.instance.N):
            for h in range(0, self.instance.H):
            ...
            for h in range(self.instance.H):
                self.model.addConstr(
                    quicksum(self.d[i, j, h, k] / self.instance.velocity[i, j, k] 
                            for i in range(self.instance.N) 
                            for j in range(self.instance.N)if i!= j) <= self.instance.e[k] - self.instance.b[k],
        ...
        for j in range(1, self.instance.N):
            self.model.addConstr(
                quicksum(self.f[i, j, h]for i in range(self.instance.N) for h in range(self.instance.H)) 
                - quicksum(self.f[j, i, h]for i in range(self.instance.N) for h in range(self.instance.H))
                == self.instance.q[j],
        ...
        for h in range(self.instance.H):
            for i in range(self.instance.N):
                for j in range(self.instance.N):
                    self.model.addConstr(
                        self.f[i, j, h]  <= self.instance.L[h] * self.y[i, j, h],
        ...
        for h in range(self.instance.H):
            for k in range(self.instance.K):
                self.model.addConstr(
                    quicksum(self.x[i, j, h, k] for i in range(self.instance.N) for j in range(self.instance.N))
                    <= 1,
        ...
        for i in range(self.instance.N):
            for j in range(self.instance.N):
                self.model.addConstr(
                    quicksum(self.y[i, j, h] for h in range(self.instance.H)) 
                    == self.X[i, j],
        ...
        for i in range(self.instance.N):
            for j in range(self.instance.N):
                for h in range(self.instance.H):
                    for k in range(self.instance.K):
                        self.model.addConstr(
                            self.y[i, j, h] >= self.x[i, j, h, k],
                            "c15:vehicle_task_constraint_[%d_%d_%d_%self.d]" % (i, j, h, k)
                        )
                    self.model.addConstr(
                        self.y[i, j, h] <= quicksum(self.x[i, j, h, k] for k in range(self.instance.K)),
    ...
    def set_objective(self):
        ...
        # 计算总运输风险
        self.total_risk = quicksum(self.x[i, j, h, k] * self.instance.risk_matrix[i, j, k] 
                      for i in range(self.instance.N) 
                      for j in range(self.instance.N) 
                      for h in range(self.instance.H) 
                      for k in range(self.instance.K) if i != j)
                    ...
                    for h in range(self.instance.H):
                        for k in range(self.instance.K):
                            if self.x[i, j, h, k].x > 0.5:
                                print("车辆%d在时段%d从%d到%d, 行驶%f, 运输%d" % (h, k, i, j, self.d[i, j, h, k].x, self.f[i, j, h].x))
